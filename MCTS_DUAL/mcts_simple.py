    ...
    def _expand_and_evaluate_with_rollout(self, node, obs_history, hidden_state, debug_info, env_state, path_actions):
        ...
        for action in sampled_actions:
            ...
            if action_key not in node.children:
                ...
                # Store prior probability under current policy.
                # Use a normalized density (exp(log_prob / action_dim)) to avoid underflow.
                action_tensor = torch.FloatTensor(action).unsqueeze(0).to(self.device)
                log_prob = dist.log_prob(action_tensor).sum()
                action_dim = max(1, int(action_tensor.shape[-1]))
                child.prior_prob = torch.exp(log_prob / action_dim).item()
