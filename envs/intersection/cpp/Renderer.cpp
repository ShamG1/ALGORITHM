...
Renderer::Renderer() {
    ...
    if(impl->hwnd){
        // Running from repo root (E:\IMPORTANT_\SCI\ALGORITHM\MCTS_DUAL), assets are under cpp\assets
        HICON icon_big = static_cast<HICON>(
            LoadImageW(nullptr, L"MCTS_DUAL\\cpp\\assets\\icon.ico", IMAGE_ICON, 0, 0,
                       LR_LOADFROMFILE | LR_DEFAULTSIZE)
        );
        HICON icon_small = static_cast<HICON>(
            LoadImageW(nullptr, L"MCTS_DUAL\\cpp\\assets\\icon.ico", IMAGE_ICON, 16, 16,
                       LR_LOADFROMFILE)
        );
        if(icon_big){
